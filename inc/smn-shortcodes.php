...
function smn_lista_categorias( $atts ) {
    extract( shortcode_atts( array(
	...
	if ( 'false' == $hide_empty ) $hide_empty = false;
	...
	if ( $terms ) {
		...
		$r .= '<ul class="wp-block-list is-style-link-list">';
		...
		foreach ( $terms as $term ) {
			...
			if ( $quitar_texto ) {
				$title = str_replace( $quitar_texto, '', $title );
			}

			$r .= '<li>
				<div class="wp-block-buttons">
					<div class="wp-block-button is-style-arrow-link d-block">
						<a class="wp-block-button__link d-block" href="'. get_term_link( $term ).'">'.$title.'</a>
					</div>
				</div>
			</li>';
...
add_shortcode( 'lista_categorias', 'smn_lista_categorias' );

function smn_post_title() {
	return esc_html( get_the_title() );
}
add_shortcode( 'post_title', 'smn_post_title' );

function smn_cta_add_to_cart() {
	...
	$r = '';
	$r .=  __( 'Precio del curso', 'smn' ) . ': <span class="font-weight-bold">' . $precio . '</span><br><br>';
	$r .= '<a class="btn btn-primary is-style-border-double" href="'. do_shortcode('[add_to_cart_url id="'.get_the_ID().'"]') .'"><img class="mr-2" src="'.get_stylesheet_directory_uri().'/img/cart.svg" alt="'.__( 'Icono del carrito', 'smn' ).'" width="24" height="24"> '.__( 'Add to cart', 'woocommerce' ).'</a>';
	return $r;
}
add_shortcode( 'cta_add_to_cart', 'smn_cta_add_to_cart' );