...
if ( function_exists( 'register_block_style' ) ) {
    ...
    //     array(
    //         'name'         => 'expanded-image',
    //         'label'        => __( 'Expandir derecha', 'smn-admin' ),
    //         'is_default'   => false,
    //     )
    // );
    ...
    register_block_style(
        ...
        array(
            'name'         => 'border-double',
            'label'        => __( 'Doble borde', 'smn-admin' ),
            'is_default'   => false,
        )
    );
...
add_filter( 'render_block', 'cover_image_header_block', 10, 2 );
function cover_image_header_block( $block_content, $block ) {

    if ( $block['blockName'] !== 'core/cover' ) return $block_content;

    if( isset( $block['attrs']['className'] ) && str_contains( $block['attrs']['className'], 'is-style-image-header' ) ) {
...
add_filter( 'render_block', 'columns_block_number', 10, 2 );
function columns_block_number( $block_content, $block ) {
    
    if ( $block['blockName'] === 'core/columns' ) {

        $num_columns = count($block['innerBlocks']);
        
        $block_content = str_replace( 
            'wp-block-columns ', 
            'wp-block-columns has-'. $num_columns .'-columns ',
            $block_content
        ); 
